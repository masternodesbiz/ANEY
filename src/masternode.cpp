...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    if (nHeight >  1000000) return   160000 * COIN;
    if (nHeight >   900000) return   140000 * COIN;
    if (nHeight >   800000) return   120000 * COIN;
    if (nHeight >   700000) return   100000 * COIN;
    if (nHeight >   600000) return    80000 * COIN;
    if (nHeight >   500000) return    60000 * COIN;
...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight >   1200000) return      10 * COIN;
    if (nHeight >   1100000) return      20 * COIN;
    if (nHeight >   1000000) return      40 * COIN;
    if (nHeight >    900000) return      60 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 ANEY tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 ANEY tx -> 1 confirmation
