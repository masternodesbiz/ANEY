...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("AnimalEconomy client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("AnimalEconomy"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("AnimalEconomy"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
